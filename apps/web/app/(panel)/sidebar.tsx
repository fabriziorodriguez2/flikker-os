...
export default function Sidebar(props: SidebarProps) {
  ...
  return (
    <aside
      ...
      className="sticky top-0 hidden h-screen w-[210px] shrink-0 flex-col border-r border-[#223247] bg-[#0D1B2A] lg:flex"
    >
      <div className="px-6 pt-6">
        <Link
          href="/dashboard"
          className="font-display text-[24px] font-bold leading-none text-[#5C6BC0]"
        >
          Flikker
        </Link>
      </div>

      <nav className="mt-7 flex flex-col gap-1 px-4">
        {NAV_ITEMS.map((item) => {
          ...
          return (
            <Link
              key={item.href}
              href={item.href}
              className={`flex min-h-8 items-center gap-3 rounded-[8px] px-3 py-2.5 text-sm font-semibold transition-colors ${
                active
