...
@Module({
  imports: [
    ...
    // `CustomerPublicUrlService` para el link de `/r/{token}` del
    // recordatorio de reseña — misma fuente de verdad que el resto de los
    // links customer-facing. No cierra ciclo: `PublicModule` no importa
    // `JobsModule`.
    PublicModule,
  ],
  providers: [
